...
<body class="font-sans tracking-widest antialiased bg-gray-300">
            ...
            <div>
                @if (session()->has('danger'))
                    <div class="p-4 mb-4 text-sm text-red-700 bg-red-100 rounded-lg " role="alert">
                        <span class="font-medium">Pogreška!</span>{{ session()->get('danger') }}
                    </div>
                @endif
                @if (session()->has('info'))
                ...
                @if (session()->has('success'))
                    <div class="p-4 mb-4 text-sm text-green-700 bg-green-100 rounded-lg " role="alert">
                        <span class="font-medium">Uspjeh!</span> {{ session()->get('success') }}
                    </div>
                @endif
                @if (session()->has('warning'))
                    <div class="p-4 mb-4 text-sm text-yellow-700 bg-yellow-100 rounded-lg" role="alert">
                        <span class="font-medium">Upozorenje!</span> {{ session()->get('warning') }}
                    </div>
                @endif
            </div>
            {{ $slot }}
